	...
	class BP_Course_Rest_User_Controller extends BP_Course_Rest_Controller {
	    ...
	    public function getmycredpoints($request){
	    	if(!function_exists('mycred') ){
	    		$data=array(
	    			'status'=>0,
	    			'points'=>0,
	    			'message'=>_x('MyCred Not Available','api','vibe')
	    		);
	    	}else{
	    		$mycred = mycred();
				...
				$data=array(
					'status'=>1,
					'points'=>$balance,
					'message'=>_x('MyCred Available','api','vibe'),
		...
		function add_user_result($request){
			...
			if(!empty($qstatus) && $qstatus > 2){
				...
				$apimessage = _x('Quiz Already Submitted!','Quiz Already submitted to serve','vibe');
			}else{
				...
				$apimessage = _x('Quiz submitted!','Quiz submitted to serve','vibe');
			}
		...
		function save_user_question($request){
			...
			$data = array('status'=>true,'message'=>_x('Marked answer saved','','vibe'));
			return 	new WP_REST_Response( $data, 200 );
		}
	    ...
	    function members_directory($request){
	    	...
	    	if(bp_is_active('members')  && function_exists('bp_core_get_users')){
		    	...
		    	$data = array(
		    		'status'=> 1,
		    		'data'=> $users,
		    		'message'=>_x('Members with filters','vibe')
		    	);
	    	}else{
	    		$data=array(
	    			'status'=>0,
	    			'data'=> [],
	    			'message'=>_x('Buddypress members component not active','vibe')
	    		);
	    	}
		...
		function get_member_profile_details($request){
			...
			if(bp_is_active('members')){
		    	...
		    	$data = array(
		    		...
		    		'message'=>_x('bp_member_api_get_user_profile_data filter','vibe')
		    	);
	    	}else{
	    		$data=array(
	    			'status'=>0,
	...
	function get_forums($request){
	    	...
	    	if(function_exists('bbp_has_forums')){
				...
				if(!empty($forums)){
					...
					$status = 1;
					$message = _x('Forums found!','Forums found!','vibe');
				}else{
					$status = 0;
					$message = _x('Forums not found!','Forums not found!','vibe');
				}
	    		...
	    		$data=array(
	    			'status' => 0,
	    			'data' => [],
	    			'message' => _x('BB-Press Plugin not active!','BB-Press Plugin not active!','vibe')
	    		);
	    	}
	    ...
	    function get_topics($request){
	    	...
	    	if(function_exists('bbp_has_topics')){
				...
				if(!empty($topics)){
					...
					$status = 1;
					$message = _x('Topics found!','Topics found!','vibe');
				}else{
					$status = 0;
					$message = _x('Topics not found!','Topics not found!','vibe');
				}
	    ...
	    function get_replies($request){
	    	...
	    	if(function_exists('bbp_has_replies')){
		    	...
		    	if ( bbp_has_replies($args) ) :
				 	while ( bbp_replies() ) : bbp_the_reply();
						...
						if(!empty($is_private) && ($user_id != $reply_post->post_author) && !user_can($user_id,'edit_posts') ){
							$reply_post->post_content = _x('Reply is set to private!','Reply is set to private!','vibe');
						}
				...
				if(!empty($replies)){
					...
					$status = 1;
					$message = _x('Replies found!','Replies found!','vibe');
				}else{
					$status = 0;
					$message = _x('Replies not found!','Replies not found!','vibe');
				}
		...
		function create_forum($request){
			...
			$data = array(
				'status' => 0,
    			'data' => [],
    			'message' => _x('Create Edit Delete Update functionality not enabled from site','BB-Press Plugin not active!','vibe')
			);
			return new WP_REST_Response($data, 200);
	    	...
	    	if(!empty($user_id)){
		    	...
		    	if(function_exists('bbp_insert_forum')){
					...
					if(!empty($flag)){
						$status = 1;
						$message = _x('Forum Created','Forum Created','vibe');
					}else{
						$status = 0;
						$message = _x('Forum not Created','Forum not Created','vibe');
					}
    			...
    			$data=array(
	    			'status' => 0,
	    			'data' => [],
	    			'message' => _x('Authorization error!','Authorization error!','vibe')
	    		);
	    	}
	    	...
	    	if(!empty($user_id)){
		    	...
		    	if(function_exists('bbp_insert_topic') ){
		    		if(!empty($topic_data['post_content'])&& !empty($topic_data['post_title']) && !empty($topic_data['post_parent']) && !empty($topic_data['post_author']) && !empty($topic_meta['forum_id'])){
						...
						if(!empty($flag)){
							$status = 1;
							$message = _x('Topic Created','Topic Created','vibe');
						}else{
							$status = 0;
							$message = _x('Topic not Created','Topic not Created','vibe');
						}
		    			...
		    			$data=array(
			    			'status' => 0,
			    			'data' => [],
			    			'message' => _x('Passing Arguments not valid!','Passing Arguments not valid!','vibe')
	    				);
		    		} 
		    	}else{
		    		$data=array(
	    		...
	    		if( !empty($reply_meta['forum_id']) && !empty($reply_meta['topic_id']) && !empty($reply_data['post_content']) && !empty($reply_data['post_parent']) && !empty($reply_data['post_author']) ){
	    			if(function_exists('bbp_insert_reply')){
						...
						if(!empty($reply_id)){
							...
							$message = _x('Reply Created','Reply Created','vibe');
						}else{
							$status = 0;
							$data = false;
							$message = _x('Reply not Created','Reply not Created','vibe');
						}
		...
		function update_reply($request){
			...
			if(!empty($reply_id)){
								...
								if(($user_id == $reply->post_author) || user_can($user_id,'edit_post') ){
									...
									if(!empty($flag)){
										...
										$data=array(
							    			'status' => 1,
							    			'data' => $ereply,
							    			'message' => _x('Reply Updated.','Reply Updated','vibe')
							    		);
									}else{
										$data=array(
							    			'status' => 0,
							    			'data' => false,
							    			'message' => _x('Reply not Updated.','Reply not Updated','vibe')
							    		);
									}
								}else{
									$data=array(
						    			'status' => 0,
						    			'data' => false,
						    			'message' =>  _x('You are not a valid user to update this reply','Not success','vibe')
					    			);
								}	
							}else{
								$data=array(
					    			'status' => 0,
					    			'data' => false,
					    			'message' => _x('Reply not Exist.','Reply not Exist','vibe')
					    		);
							}
			    		}else{
			    			$data=array(
				...
				$data=array(
	    			'status' => 0,
	    			'data' => false,
	    			'message' => _x('Insufficient data','Insufficient data','vibe')
	    		);
			}
					...
					if ( empty( $reply ) ){
						...
						if($user_id == $reply->post_author){
							switch ( $sub_action ) {
								...
								if($success){
									$message = _x('Reply trash successfull','Reply trash successfull','vibe');
								}
								break;
							...
							case 'untrash':
								...
								if($success){
									$message = _x('Reply untrash successfull','Reply untrash successfull','vibe');
								}
								break;
							...
							case 'delete':
								...
								if($success){
									$message = _x('Reply delete successfull','Reply delete successfull','vibe');
								}
								break;
							}
							...
							if($success){
								...
								$data=array(
					    			'status' => 0,
					    			'data' => $success,
					    			'message' =>  _x('Not success','Not success','vibe')
				    			);
							}
						}else{
							$data=array(
				    			'status' => 0,
				    			'data' => false,
				    			'message' =>  _x('You are not a valid user to delete this reply','Not success','vibe')
			    			);
						}	
					}
		    	}else{
		...
		function set_registrationId_push_notification($request){
	    	...
	    	if(!empty($post['registrationId'])){
				...
				$data =array(
					'status' => 1,
					'message' => _x('Device registered for Push Notification','push notification','vibe')
				);
	    	}else{
	    		$data = array(
	    			'status' => 0,
	    			'message' => _x('RegistrationId not found please Re-open app again','push notification','vibe')
	    		);
	    	}
		...
		function delete_registrationId_push_notification($request){
	    	...
	    	if(!empty($post['registrationId'])){
				...
				$data =array(
					'status' => 1,
					'message' => _x('Device removed for Push Notification','push notification','vibe')
				);
	    	}else{
	    		$data = array(
	    			'status' => 0,
