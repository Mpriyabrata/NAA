jQuery(document).ready(function ($) {
		...
		$.ajax({
			...
			success: function (response) {
				...
				if (response.error) {
					...
					$("#statusmessage").appendTo('#fullsection_messages').removeClass('notice-error').addClass('notice-success').html('<p style="float:left !important; margin-top:-15px!important;">' + fullcontent_settings_object.statusmessage2 + '</p>').show().delay(5000).fadeOut('slow');
					//active buttons
					$(".fullcontent-saveass").find('span').text('New');
					$(".fullcontent-saveass").removeClass('button-disabled');
					$("#mff_inspector").removeClass('button-disabled');
			...
			$.ajax({
				...
				success: function (response) {
					...
					if (response.error) {
						...
						response.filename = response.filename.replace('.txt', '');
						$('input[name="filename"]').val(response.filename);
						$('#savemessage').removeClass('notice-error').addClass('notice-success').html('<p style="float:left !important; margin-top:-15px!important; font-size:1.4em !important; ">' + fullcontent_settings_object.savemessage + '</p>').show().delay(5000).fadeOut('slow');
					}
					$('#saving .spinner').remove();
					$("#fullsection_messages #section_spinner .spinner").remove();
	...
	$(document.body).on('click', '.addfile', function (e) {
		...
		if ($('#filename').attr('readonly') == 'readonly') {
			$("#savingas").show();
			$('#savingas').html('<p style="float:left !important; margin-top:-15px!important; font-size:1.2em !important;">' + fullcontent_settings_object.savingas + '</p>').show();
			$(this).attr('title', fullcontent_settings_object.savingastitle).show();
			$('#filename').removeAttr('readonly');
			$('#filename').focus().select();
		} else {
			$("#savingas").show();
			$('#savingas').html('<p style="float:left !important; margin-top:-15px !important; font-size:1.4em !important;">' + fullcontent_settings_object.savingas2 + '</p>');
			$(this).attr('title', '');
			...
			$.ajax({
				...
				success: function (response) {
					...
					if (response.error) {
						...
						$('input[name="pathfilename"]').val(response.pathfilename);
						$('input[name="filename"]').val(response.filename);
						$('#savingas').html('<p style="float:left !important; margin-top:-15px!important; font-size:1.4em !important;">' + fullcontent_settings_object.savemessage + '</p>').show().delay(5000).fadeOut('slow');
						submitButton.removeClass('button-disabled');
						location.reload();
					}
					$('#saving .spinner').remove();
	...
	$(document.body).on('click', '.preview_txt', function (e) {
		...
		//$('#saving').append( '<span class="spinner is-active"></span>' );
		$("#fullsection_messages #section_spinner").prepend('<span class="spinner is-active" style="float:left;"></span>');
		$('#savemessage').removeClass('notice-error').addClass('notice-success').html('<p style="float:left !important; margin-top:-15px !important; font-size:1.4em !important;">' + fullcontent_settings_object.testing + '</p>').show().delay(5000).fadeOut('slow');
		...
		$.ajax({
			...
			success: function (response) {
				...
				if (response.error) {
					...
					$('#savemessage').removeClass('notice-error').addClass('notice-success').html('<p style="float:left !important; margin-top:-15px !important; font-size:1.4em !important;">' + fullcontent_settings_object.tested + '</p>').show().delay(5000).fadeOut('slow');
					//$('#savingas').html('').fadeOut('slow');
					$('.spinner').remove();
				}
			}
		}).fail(function (response) {
			$('#savemessage').removeClass('notice-success').addClass('notice-error').html('<p style="float:left !important; margin-top:-15px !important; font-size:1.4em !important;">ERROR: ' + fullcontent_settings_object.goeswrong + '</p>').show().delay(5000).fadeOut('slow');
			if (window.console && window.console.log) {
				console.log(response);
			}
			$('.spinner').remove();
